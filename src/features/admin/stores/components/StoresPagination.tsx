...
import type { AdminStoresMeta } from "../types/stores.types";

export interface StoresPaginationProps {
...
export function StoresPagination({
  ...
  const { page, total, totalPages, hasPreviousPage, hasNextPage } = meta;
  const startRecord = total === 0 ? 0 : (page - 1) * rowsPerPage + 1;
  const endRecord = Math.min(page * rowsPerPage, total);

  // Generate pagination pills matching screenshot: 1, 2, 3, ..., 537
    ...
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-5 py-3.5 border-t border-admin-line text-[13px] text-admin-text-soft select-none">
      ...
      <div className="flex items-center gap-2">
        ...
        <select
          ...
          className="rounded-[8px] border border-admin-line bg-admin-surface px-2 py-1 text-[13px] text-admin-text outline-none cursor-pointer hover:border-admin-brand/40 transition-colors"
        >
      ...
      <div className="flex items-center gap-1.5">
        <button
          ...
          className="px-1.5 py-1 text-[13px] text-admin-text-soft hover:text-admin-text disabled:opacity-40 disabled:pointer-events-none transition-colors cursor-pointer"
        >
          &lt; Prev
        </button>
